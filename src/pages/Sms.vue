        ...
        <div class="bg-[#F4F6F6] rounded-[8px] p-[6px] mt-[16px]">
          ...
          <div class="pt-[6px]">
            <table class="sms-table">
              ...
              <tbody>
                <tr @click="openMessage( body.id )" v-for="(body, index) in paginatedData" :key="index" class="text-[14px]" style="font-family: Geist; font-weight: 600; color: #000000">
                  <td>{{ body.id }}</td>
                  <td class="w-[217px]">{{ body.client }}</td>
                  <td class="text-[12px]" style="color: #72908d">{{ body.phone_number }}</td>
                  <td class="w-[415px]">{{ truncateText(body.message_text, 120) }}</td>
                  <td class="text-[#72908D]">{{ body.sent_date }}</td>
                  <td>{{ body.status }}</td>
                  <td>
                    <button v-if="index % 3 == 0" class="py-[10px] w-[160px] bg-[#C5F7DC4D] rounded-[6px]" style="font-family: Geist; font-weight: 600; color: #008e42">{{ body.sms_status }}</button>
                    <button v-else-if="index % 3 == 1" class="py-[10px] w-[160px] bg-[#E5EBFF] rounded-[6px]" style="font-family: Geist; font-weight: 600; color: #0037ff">{{ body.sms_status }}</button>
                    <button v-else class="py-[10px] w-[160px] bg-[#E5EBFF] rounded-[6px]" style="font-family: Geist; font-weight: 600; color: #0037ff">{{ body.sms_status }}</button>
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        <div class="bg-[#F4F6F6] p-[6px] rounded-[8px] w-[600px]">
          <div class="bg-[#FFFFFF] py-[16px] px-[12px] rounded-[6px]">
            ...
            <div class="py-[16px]">
              ...
              <p class="text-left text-[14px]" style="font-family: Geist; font-weight: 500; color: #4f7471">
                Hurmatli ABROROV ULMASJON ULUG‘BEKOVICH, siz ATLANT IDEAL INVESTdan xarid qilingan 24 raqamli xonadon uchun 6215000 so'm miqdorida to'lov amalga oshirdingiz! Qoldiq: 226459000.00 so'm, To'lov qilish raqami: 230750095
              </p>
            </div>
            <div class="divide"></div>
              ...
              <div class="pt-[24px]">
                <ul class="flex gap-[12px]">
                  <li
                    ...
                    v-for="(button, index) in notificationTabs"
                    :key="index"
              ...
              <div v-if="activeButton === 1" class="pt-[24px]">
                <div>
                  ...
                  <div class="pt-[24px]">
                    <p>Template</p>
                    <div class="flex items-center gap-[12px]">
                      <a-select class="mt-[12px]" v-model:value="value1" :size="size" style="width: 435px" :options="options"></a-select>
                  ...
                  <div class="pt-[24px]">
                    <div class="w-full rounded-[6px] py-[6px] px-[12px] h-[145px] flex flex-wrap items-start gap-[8px] overflow-auto" style="border: 1px solid #dce3e3">
                      <div class="py-[6px] px-[10px] bg-[#E5EBFF] rounded-[6px] text-[#0037FF] text-[12px] cursor-pointer flex items-center gap-[6px]" v-for="(tag, index) in selectedTags" :key="index" style="font-family: Geist; font-weight: 600">
                        {{ tag }}
                    ...
                    <div class="flex items-center flex-wrap gap-[8px] w-full pt-[24px]">
                      <button class="py-[6px] px-[10px] bg-[#E5EBFF] rounded text-[#0037FF] text-[12px] cursor-pointer" v-for="(tag, index) in tags" :key="index" @click="selectTag(tag)" style="font-family: Geist; font-weight: 600">
                        {{ tag }}
                      </button>
                    </div>
                  </div>
...
export default {
  ...
  methods: {
    ...
    async fetchTableData() {
      try {
        const response = await axios.get("/common/sms/");
